...
- **Reason skipped**: no test file renders `useHandoffDialogState` — `AgentSessionContinuationDialog.test.tsx` mocks the hook wholesale, so the hook has zero coverage today. It reads the app store through three `useAppStore` selectors plus `getState`, and drives capture, target resolution, secret scanning, and draft preservation on mount, so a `renderHook` harness means standing up a full store fixture and mocking eight modules. The BUG-6 fix (only adopt the newly saved template when nothing is selected) is anchored by typecheck and by `HandoffNotesControls.test.tsx` on the component side, but the selection rule itself is unproven.
