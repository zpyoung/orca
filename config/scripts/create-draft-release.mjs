...
export async function createDraftRelease({
  ...
  if (fullBody.length !== body.length) {
    log(
      `${existingRelease ? 'Updated' : 'Created'} draft release ${tag} with truncated ${source} (${body.length} chars).`
    )
  } else {
    log(
      `${existingRelease ? 'Updated' : 'Created'} draft release ${tag} with ${source} (${body.length} chars).`
    )
  }
}
