// Forked from src/main/artifacts/artifact-cloud-service.ts at da585e8b14.
...
import { ArtifactPasswordCoordinator } from './artifact-password-coordinator'
...
export class ArtifactCloudService {
  ...
  private readonly passwords: ArtifactPasswordCoordinator
  ...
  constructor(
    ...
    this.passwords = new ArtifactPasswordCoordinator(userDataPath)
  }
  ...
  list(options: ArtifactListOptions): Promise<ArtifactCloudOperation<ArtifactListPage>> {
    return this.withAuth(options, async (token, apiUrl, auth) => {
      await this.passwords.recover(token, apiUrl, auth, this.publisher)
      const query = options.cursor ? `?cursor=${encodeURIComponent(options.cursor)}` : ''
      const page = await artifactRequest<ArtifactListPage>(apiUrl, token, query)
      return this.passwords.list(page, auth)
    })
  }
  ...
  getPublishedLink(
    ...
    return this.withAuth(request, async (_token, _apiUrl, auth) => {
      ...
      return record
        ? this.passwords.publishedLink(
            record.shareUrl,
            request.sourceKey,
            request.revealPassphrase === true,
            auth
          )
        : null
    })
  ...
  async share(request: ArtifactWriteRequest): Promise<ArtifactCloudOperation<ArtifactListItem>> {
    ...
    return this.withAuth(request, (token, apiUrl, auth) =>
      this.passwords.share(request, token, apiUrl, auth, idempotencyKey, this.publisher)
    )
  }
  ...
  async publish(
    ...
    const idempotencyKey = randomUUID()
    return this.withAuth(request, (token, apiUrl, auth) =>
      this.passwords.publish(request, token, apiUrl, auth, idempotencyKey, this.publisher)
    )
  }
  ...
  async update(request: ArtifactWriteRequest): Promise<ArtifactCloudOperation<ArtifactListItem>> {
    assertArtifactSharingAllowed(this.isSharingEnabled)
    return this.withAuth(request, (token, apiUrl, auth) =>
      this.passwords.update(request, auth, (preparedRequest) =>
          ...
          return this.publisher.runForSlug(record.slug, auth, async () => {
            ...
            const response = await artifactRequest<ArtifactListItem>(
              ...
              `/${record.slug}`,
              {
                ...
                body: artifactWriteBody(preparedRequest)
              }
            )
            auth.assertCurrent()
            refreshArtifactShareRecordExpiration(
  ...
  unshare(
    ...
    return this.withAuth(request, (token, apiUrl, auth) =>
      this.passwords.runForLifecycleSource(request.sourceKey, auth, () =>
        this.publisher.runForSource(request.sourceKey, auth, async () => {
          auth.assertCurrent()
          const record = getArtifactShareRecord(
            auth.profileId,
          ...
          await this.passwords.ensureRotationCleanupBeforeForget(
          ...
          return this.publisher.runForSlug(record.slug, auth, async () => {
            ...
            this.passwords.forget(auth, { sourceKey: request.sourceKey, slug: record.slug })
          })
        })
      )
    )
  ...
  delete(id: string, options: ArtifactCloudOptions): Promise<ArtifactCloudOperation<void>> {
    return this.withAuth(options, (token, apiUrl, auth) =>
      this.passwords.runForLifecycleSlug(id, auth, () =>
        this.publisher.runForSlug(id, auth, async () => {
          ...
          this.passwords.forget(auth, { slug: id })
        })
      )
    )
  }
