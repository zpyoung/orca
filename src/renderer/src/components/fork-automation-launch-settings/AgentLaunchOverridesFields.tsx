...
export type AgentLaunchOverridesFieldsProps = {
  ...
  /** False when the launch reads no CLI arguments (e.g. a structured native chat session); omits the field rather than disabling it. */
  showAgentArgs?: boolean
  className?: string
  idPrefix: string
}
...
export function AgentLaunchOverridesFields(
  ...
  const showAgentArgs = props.showAgentArgs ?? true
  const rawArguments = showAgentArgs ? (
    <RawArgumentsField
