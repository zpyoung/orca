...
const CLAUDE_MODEL_OPTION_ALIASES = ['--model'] as const
...
function applyRecipeOptionOverride(args: {
  ...
  const recipeOption = recipeOptions.at(-1)
...
export function applyCommitMessageRecipeOptionOverrides(
  agentId: TuiAgent,
  generatedArgs: string[],
  recipeArgs: string[]
): { generatedArgs: string[]; recipeArgs: string[] } {
  const options =
    agentId === 'claude'
      ? [{ aliases: CLAUDE_MODEL_OPTION_ALIASES }, { aliases: CLAUDE_EFFORT_OPTION_ALIASES }]
      : agentId === 'codex'
        ? [
